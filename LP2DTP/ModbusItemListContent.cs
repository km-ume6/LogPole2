...
namespace LP2DTP
{
    public sealed class ModbusItemListContent : Grid
    {
        ...
        private Grid CreateHeaderRow()
        {
            ...
            var sortColumns = new[] { "MachineName", "UnitName", "IpAddress", "UnitId", "RegAddress", "FunctionCode", "RegCount", "ByteOrder", "ItemType", "Enabled", "" };
        ...
        private void SortByColumn(string columnName)
        {
            ...
            var sortedItems = columnName switch
            {
                ...
                "UnitId" => _isAscending
                    ? ViewModel.Items.OrderBy(x => x.UnitId).ToList()
                    : ViewModel.Items.OrderByDescending(x => x.UnitId).ToList(),
                "RegAddress" => _isAscending
                    ? ViewModel.Items.OrderBy(x => x.TemperatureRegisterAddress).ToList()
                    : ViewModel.Items.OrderByDescending(x => x.TemperatureRegisterAddress).ToList(),
                "FunctionCode" => _isAscending
                    ? ViewModel.Items.OrderBy(x => x.FunctionCode).ToList()
                    : ViewModel.Items.OrderByDescending(x => x.FunctionCode).ToList(),
                "RegCount" => _isAscending
                    ? ViewModel.Items.OrderBy(x => x.RegisterCount).ToList()
                    : ViewModel.Items.OrderByDescending(x => x.RegisterCount).ToList(),
                "ByteOrder" => _isAscending
                    ? ViewModel.Items.OrderBy(x => x.ByteOrder).ToList()
                    : ViewModel.Items.OrderByDescending(x => x.ByteOrder).ToList(),
